...
namespace Human.Chrs.Domain.Helper
{
    public static class CryptHelper
    {
        ...
        public static string SaltHashPlus(string data)
        {
            byte[] salt = new byte[16];
            byte[] hashed = new byte[64];  // SHA-3 512-bit
            ...
            IDigest digest = new KeccakDigest(512);
            digest.BlockUpdate(salt, 0, salt.Length);
            digest.BlockUpdate(System.Text.Encoding.UTF8.GetBytes(data), 0, data.Length);
            digest.DoFinal(hashed, 0);

            Buffer.BlockCopy(salt, 0, result, 1, 16);
            Buffer.BlockCopy(hashed, 0, result, 17, 64);

            return Convert.ToBase64String(result);
        }

        public static bool VerifySaltHashPlus(string hashedData, string data)
        {
            if (hashedData == null || data == null)
            {
                return false;
            }

            byte[] check = Convert.FromBase64String(hashedData);
            if (check[0] != 0 || check.Length != 81)
            {
                return false;
            }

            byte[] salt = new byte[16];
            Buffer.BlockCopy(check, 1, salt, 0, 16);

            byte[] hashed = new byte[64];
            Buffer.BlockCopy(check, 17, hashed, 0, 64);

            byte[] checkHashed = new byte[64];

            IDigest digest = new KeccakDigest(512);
            digest.BlockUpdate(salt, 0, salt.Length);
            digest.BlockUpdate(System.Text.Encoding.UTF8.GetBytes(data), 0, data.Length);
            digest.DoFinal(checkHashed, 0);
